...
const (
	codexUserAgent             = "codex_exec/0.157.0 (Ubuntu 20.4.0; x86_64) xterm-256color (codex_exec; 0.157.0)"
...
func applyCodexHeadersFromSources(r *http.Request, auth *cliproxyauth.Auth, token string, stream bool, cfg *config.Config, ginHeaders http.Header) {
	...
	} else if strings.TrimSpace(cfgBetaFeatures) != "" {
		r.Header.Set("X-Codex-Beta-Features", cfgBetaFeatures)
	}
